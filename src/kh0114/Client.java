...
public class Client implements ActionListener, Runnable {
	...
	public Client() {
		...
		// 아래에 붙는 판넬 코드
		jp1 = new JPanel(); // 아래에 붙는 판넬
		jp1.setLayout(new BorderLayout()); // 동서남북중앙 레이아웃
		jtf = new JTextField(30); // 30글자
		jbtn = new JButton("종료"); // 종료버튼 생성
		jp1.add(jbtn, BorderLayout.EAST);
		jp1.add(jtf, BorderLayout.CENTER);
		// 위쪽에 붙는 판넬 코드
		jp2 = new JPanel(); // 위에 붙는 판넬
		...
		//프레임에 붙이는 코드
		jta = new JTextArea("", 10, 50); // 초기값, 행(세로), 열(가로)
		jta.setBackground(Color.GREEN); // 대화창 배경색 설정
	...
	public void actionPerformed(ActionEvent e) { // 이벤트처리
		Object obj = e.getSource(); // 이벤트 발생위치 얻기
		String msg = jtf.getText(); // 채팅 내용 입력 받기
		if(obj == jtf) { // 입력란에서 엔터를 친경우
	...
	public void run() {
		...
		while (! isStop) {
			try {
				message = (String)br_in.readLine(); // 채팅 내용

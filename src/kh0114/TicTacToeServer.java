...
public class TicTacToeServer {
    public static void main(String[] args) throws Exception {
        ...
        //경기자가 2명 모이면 새로운 게임이 시작된다. 게임은 얼마든지 많이 생성될 수 있다.
        try {
            while (true){
        ...
        try {
            ...
            while (true){
                ...
                if (command.startsWith("MOVE")){//MOVE 명령어
                    int i = Integer.parseInt(command.substring(5,6));//문자열을 숫자로 변환
                    int j = Integer.parseInt(command.substring(7,8));//문자열을 숫자로 변환
                    game.setBoards(i,j,playerMark);
                    game.printBoard();
                    other.output.println("OTHER " + i + " " + j);
                    output.println("PRINT 기다리세요!");
                    other.output.println("PRINT 당신 차례입니다.");
                }else if (command.startsWith("QUIT")){//QUIT명령어
                    return;
                }
            }
        }catch (IOException e){
